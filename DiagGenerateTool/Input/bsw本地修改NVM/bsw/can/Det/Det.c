...
#include "Det.h"
#include "Compiler.h"
#if defined(DET_SECOC_PRIVATE_CODE)
...
#if defined(DET_SECOC_PRIVATE_CODE)
FUNC(Std_ReturnType, DET_CODE) SecOC_Det_ReportError( uint8 ApiId, uint8 error_code )
{
	uint8 ret = 0;
	if(error_code == SECOC_E_NO_ERROR ||
			error_code == SECOC_E_UNINIT || error_code == SECOC_E_INVALID_PDU_SDU_ID ||
			error_code == SECOC_E_CRYPTO_FAILURE || error_code == SECOC_E_PARAM ||
			error_code == SECOC_E_SECURED_AREA)
	{
		if(SecOC_IsDevModeEnabled() != TRUE)
		{
		Dem_ReportErrorStatus(DemConf_DemEventParameter_SecOC_software_module_running_error_Unexpected_Operation,DEM_EVENT_STATUS_FAILED);
		}
		return ret;
	}
}

FUNC(Std_ReturnType, DET_CODE) SecOC_Det_ReportRuntimeError( uint8 ApiId, uint8 error_code )
{
	uint8 ret = 0;
	if(error_code == SECOC_E_NO_ERROR ||
			error_code == SECOC_E_UNINIT || error_code == SECOC_E_INVALID_PDU_SDU_ID ||
